...
class Application @Inject()(gitHub: GitHub, bower: Bower, npm: NPM, heroku: Heroku, pusher: Pusher, cache: Cache, mavenCentral: MavenCentral, npmWebJar: NPMWebJar, bowerWebJar: BowerWebJar, webJarsFileService: WebJarsFileService, actorSystem: ActorSystem, configuration: Configuration, environment: Environment) (implicit ec: ExecutionContext, staticWebJarAssets: StaticWebJarAssets) extends Controller {
  ...
  def index = Action.async { request =>
    sortedMostPopularWebJars.map(maybeCached(request, webJars => Ok(views.html.index(webJars)))).recover {
      case e: Exception =>
    ...
    Action.async { implicit request =>
      webJarsWithTimeout().map {
        maybeCached(request, { webJars =>
          render {
            case Accepts.Html() => Ok(views.html.all(webJars))
        ...
        case e: Exception =>
          render {
            case Accepts.Html() => InternalServerError(views.html.all(Seq.empty[WebJar]))
